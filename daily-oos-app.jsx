  ...
  return(<div style={S.app}>
    ...
    {confirmDlg&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,direction:"rtl"}}>
      <div style={{background:"#1e293b",borderRadius:16,padding:"24px 20px",maxWidth:360,width:"100%",border:"1px solid rgba(255,255,255,0.1)",boxShadow:"0 20px 60px rgba(0,0,0,0.5)"}}>
        ...
        {confirmDlg.type==="extra"&&<>
          ...
          <div style={{fontSize:14,color:"#e2e8f0",lineHeight:1.8,marginBottom:12}}>الحقول التالية ناقصة:</div>
          <div style={{marginBottom:16}}>
            {confirmDlg.missing.map(m=><div key={m} style={{fontSize:13,color:"#f87171",padding:"6px 12px",background:"rgba(239,68,68,0.1)",borderRadius:8,marginBottom:4,border:"1px solid rgba(239,68,68,0.15)"}}>• {m}</div>)}
          </div>
          <div style={{display:"flex",gap:10}}>
    ...
    {pg==="entry"&&cust&&<div>
        ...
        {PG.map(g=>{const col=g.color;const cf=g.items.filter(i=>qty[i.o]!=null&&qty[i.o]!=="").length;const isE=expCat===g.cat;
          return(<div key={g.cat} style={{marginBottom:3}}>
            <div style={S.ch(col)} onClick={()=>{
                if(isE){
                  // Closing: check unfilled items
                  const unfilled=g.items.filter(i=>qty[i.o]==null||qty[i.o]==="");
                  if(unfilled.length>0&&unfilled.length<g.items.length){
                    setConfirmDlg({type:"cat",cat:g.cat,count:unfilled.length,total:g.items.length,onYes:()=>{setExpCat(null);setConfirmDlg(null)},onNo:()=>setConfirmDlg(null)});
                  }else{setExpCat(null)}
            ...
            <div style={{...S.cd,marginTop:10,borderColor:"rgba(139,92,246,0.15)"}}>
              ...
              <div style={{display:"flex",gap:8,marginTop:12}}>
                <div style={{flex:1}}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4}}>رفوف بونجورنو</label><input style={S.inp} type="number" min="0" value={ex.bonShelf||""} onChange={e=>setEx(p=>({...p,bonShelf:e.target.value}))}/></div>
                <div style={{flex:1}}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4}}>رفوف القهوة</label><input style={S.inp} type="number" min="0" value={ex.coffeeShelf||""} onChange={e=>setEx(p=>({...p,coffeeShelf:e.target.value}))}/></div>
              </div>
              ...
              <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4,marginTop:12}}>مخالفات الثلاجة</label>
              <input style={S.inp} value={ex.chillerNotes||""} onChange={e=>setEx(p=>({...p,chillerNotes:e.target.value}))} placeholder="اكتب المخالفات..."/>
      ...
      <button style={{...S.sub,background:editId?"linear-gradient(135deg,#d97706,#b45309)":"linear-gradient(135deg,#2563eb,#1d4ed8)"}} onClick={()=>{
              const missing=[];
              if(!ex.status)missing.push("حالة العميل");
              if(!ex.bonShelf&&ex.bonShelf!==0)missing.push("رفوف بونجورنو");
              if(!ex.coffeeShelf&&ex.coffeeShelf!==0)missing.push("رفوف القهوة");
              if(!ex.chiller)missing.push("ثلاجة كيت كات");
              if(missing.length>0){
                setConfirmDlg({type:"extra",missing,onYes:()=>{submit();setConfirmDlg(null)},onNo:()=>setConfirmDlg(null)});
              }else{submit()}
