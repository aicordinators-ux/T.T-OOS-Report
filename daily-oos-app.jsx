...
function doExport(subs) {
  ...
  subs.forEach(sub=>{
    ...
    allItems.forEach(item=>{
      ...
      data.push([cu.c||"",cu.r||"",cu.d||"",cu.ac||"",cu.bc||"",new Date(sub.date).toLocaleDateString("en-GB"),cu.n||"",cu.a||"",item.co,item.ca,item.e,qtyVal,sub.extra?.status||"",cu.bs||"",sub.extra?.bonShelf||0,"","","",sub.extra?.notesMansour||"",sub.extra?.chiller||"",sub.extra?.chillerNotes||"",...visVals,sub.extra?.notes||"",cu.dv||"",cu.tp||"",cu.dt||"",cu.tc||"",cu.bt||"",cu.m||"",cu.tl||""]);
    });
  });
...
export default function App(){
  ...
  return(<div style={S.app}>
    ...
    {pg==="entry"&&cust&&<div>
      <div style={{...S.cd,background:"linear-gradient(135deg,rgba(16,185,129,0.06),rgba(6,182,212,0.04))",borderColor:"rgba(16,185,129,0.12)"}}><div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontWeight:700,fontSize:15}}>{cust.n}{editId&&<span style={{fontSize:11,color:"#f59e0b",marginRight:6}}>📝 تعديل</span>}</div><div style={{fontSize:11,color:"#64748b"}}>#{cust.c} • {cust.bt} • {cust.m}</div></div><button onClick={()=>{setPg(editId?"reports":"home");setCust(null);setEditId(null);setQty({});setVis({});setEx({})}} style={{background:"none",border:"none",color:"#60a5fa",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{editId?"← رجوع":"✕ إلغاء"}</button></div></div>
      <div style={{display:"flex",gap:4,marginBottom:8}}><button style={S.tr(tab==="oos")} onClick={()=>setTab("oos")}>📦 OOS<br/><span style={{fontSize:10,fontWeight:400}}>{filled}/{totalI}</span></button><button style={S.tr(tab==="vis")} onClick={()=>setTab("vis")}>👁️ Vis<br/><span style={{fontSize:10,fontWeight:400}}>{visAch}/{visReq}</span></button><button style={S.tr(tab==="extra")} onClick={()=>setTab("extra")}>📋 إضافية</button></div>
      ...
      {tab==="extra"&&<div style={S.cd}>
        <div style={{fontSize:13,fontWeight:600,marginBottom:10,color:"#94a3b8"}}>📋 بيانات إضافية</div>
        ...
        <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4,marginTop:12}}>مساحة رفوف بونجورنو</label><input style={S.inp} type="number" value={ex.bonShelf||""} onChange={e=>setEx(p=>({...p,bonShelf:e.target.value}))}/>
        <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4,marginTop:12}}>إجمالى رفوف القهوة</label><input style={S.inp} type="number" value={ex.coffeeShelf||""} onChange={e=>setEx(p=>({...p,coffeeShelf:e.target.value}))}/>
        <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4,marginTop:12}}>ثلاجة كيت كات</label><select style={S.sel} value={ex.chiller||""} onChange={e=>setEx(p=>({...p,chiller:e.target.value}))}><option value="">اختر...</option><option>يوجد</option><option>لا يوجد</option></select>
        <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4,marginTop:12}}>مخالفات الثلاجة</label><input style={S.inp} value={ex.chillerNotes||""} onChange={e=>setEx(p=>({...p,chillerNotes:e.target.value}))}/>
        <label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:4,marginTop:12}}>ملاحظات</label><textarea style={{...S.inp,height:60,resize:"none"}} value={ex.notes||""} onChange={e=>setEx(p=>({...p,notes:e.target.value}))}/>
      </div>}
      <button style={{...S.sub,background:editId?"linear-gradient(135deg,#d97706,#b45309)":"linear-gradient(135deg,#2563eb,#1d4ed8)"}} onClick={submit}>{editId?"💾 تحديث التقرير":"💾 حفظ التقرير"}</button>
    </div>}
