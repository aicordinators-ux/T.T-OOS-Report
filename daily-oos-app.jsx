...
function getVisVal(k, vis) {
  ...
  // Shelf items (fitnessShelf, tolaShelf, rtdShelf)
  if(k==="fitnessShelf"||k==="tolaShelf"||k==="rtdShelf") {
    return vis?.[k+"_a"]||0;
  }
  // Toggle items with count
  const v = vis?.[k];
  if(v==="achieved") return Number(vis?.[k+"_cnt"])||1;
  return 0;
}
  ...
  return(<div style={{marginBottom:5}}>
    ...
    {open&&<div style={{background:"rgba(0,0,0,0.12)",borderRadius:8,overflow:"hidden",marginTop:2}}>
      {items.map(item=>{const cv=contract?.[item.key];const isReq=cv!=null;const val=vis[item.id];const isA=val==="achieved";const isN=val==="not_achieved";
        ...
        return(<div key={item.id} style={{display:"flex",alignItems:"center",gap:5,padding:"7px 10px",borderBottom:"1px solid rgba(255,255,255,0.03)",flexWrap:"wrap"}}><div style={{flex:1,minWidth:100}}><div style={{fontSize:12,color:isReq?"#e2e8f0":"#94a3b8"}}>{item.label}</div>{isReq&&<span style={{fontSize:10,color:"#f59e0b",background:"rgba(245,158,11,0.15)",padding:"1px 7px",borderRadius:8}}>📋 مطلوب</span>}{!isReq&&<span style={{fontSize:9,color:"#475569"}}>غير مطلوب</span>}</div><button onClick={()=>setVis(p=>({...p,[item.id]:isA?null:"achieved"}))} style={{...tb,borderColor:isA?"#22c55e":"rgba(255,255,255,0.08)",background:isA?"rgba(34,197,94,0.15)":"transparent",color:isA?"#4ade80":"#64748b"}}>✓</button><button onClick={()=>setVis(p=>({...p,[item.id]:isN?null:"not_achieved"}))} style={{...tb,borderColor:isN?"#ef4444":"rgba(255,255,255,0.08)",background:isN?"rgba(239,68,68,0.15)":"transparent",color:isN?"#f87171":"#64748b"}}>✗</button>{isA&&<input type="number" min="1" style={{width:50,padding:"4px 6px",borderRadius:6,border:"1px solid rgba(34,197,94,0.3)",background:"rgba(0,0,0,0.3)",color:"#4ade80",fontSize:12,fontFamily:"inherit",outline:"none",textAlign:"center",marginRight:4}} placeholder="عدد" value={vis[item.id+"_cnt"]||""} onChange={e=>setVis(p=>({...p,[item.id+"_cnt"]:e.target.value}))}/>}</div>);
      })}
    </div>}
  </div>);
}
  ...
  return(<div style={S.app}>
      ...
      <div style={{background:"#1e293b",borderRadius:16,padding:"24px 20px",maxWidth:360,width:"100%",border:"1px solid rgba(255,255,255,0.1)",boxShadow:"0 20px 60px rgba(0,0,0,0.5)"}}>
        ...
        {confirmDlg.type==="full"&&<>
          ...
          <div style={{maxHeight:300,overflowY:"auto",marginBottom:16}}>
            ...
            {(confirmDlg.missingVis>0||confirmDlg.missingPct>0||confirmDlg.noCountVis?.length>0)&&<div style={{marginBottom:10}}>
              ...
              {confirmDlg.noCountVis?.length>0&&<div style={{fontSize:12,color:"#f59e0b",padding:"5px 10px",background:"rgba(245,158,11,0.08)",borderRadius:6,marginBottom:3}}>• {confirmDlg.noCountVis.length} بند متطبق بدون إدخال عدد</div>}
            </div>}
    ...
    {pg==="entry"&&cust&&<div>
      ...
      <button style={{...S.sub,background:editId?"linear-gradient(135deg,#d97706,#b45309)":"linear-gradient(135deg,#2563eb,#1d4ed8)"}} onClick={()=>{
              ...
              // Check achieved items without count
              const noCountVis=allVisItems.filter(i=>vis[i.id]==="achieved"&&!vis[i.id+"_cnt"]);
              // Pct items
              ...
              const hasIssues=incompleteCats.length>0||emptyCats.length>0||missingVis.length>0||missingPct.length>0||missingExtra.length>0||noCountVis.length>0;
              if(hasIssues){
                setConfirmDlg({type:"full",incompleteCats,emptyCats,missingVis:missingVis.length,missingPct:missingPct.length,missingExtra,noCountVis,
